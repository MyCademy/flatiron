...
module.exports = function create(name, type, callback) {
  ...
  if (!existsSync(scaffold)) {
    console.log('Scaffold named ' + type.yellow + ' does not exist');
    return callback();
  }
  ...
  function createDirs(next) {
    ...
    Object.keys(dirs).forEach(function (name) {
      console.log('Creating directory ' + name.grey);
    });
  ...
  function createFiles(next) {
    ...
    function copyFile(file, nextFile) {
      console.log('Writing file ' + file.grey);
  ...
  function createPackage(next) {
    ...
    console.log('Writing ' + 'package.json'.grey);
    fs.writeFile(path.join(root, 'package.json'), JSON.stringify(pkg, null, 2) + '\n', next);
  }

    console.log('Creating application ' + (name ? name.magenta : ''));
    console.log('Using ' + type.yellow + ' scaffold.');
  common.async.series([
    prompt,
    createDirs,
    createPackage,
    createFiles
      ...
      console.log('Application ' + info.name.magenta + ' is now ready');
      callback();
    }
  );
}
